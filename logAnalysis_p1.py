...
# Function execute the recieved query
# and send the result back to the sender function
...
def get_authors():
        ...
        query = """select name, count(status) as numOfViews
        from log, articles, authors
        where authors.id = articles.author and
        (select substring(log.path, 10) = articles.slug)
        group by name
...
def get_errors():
        ...
        query = """select date, errorPer from errorPercentage
        where errorPer > 1;"""
